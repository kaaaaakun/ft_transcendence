...
const Pong = () => {
  ...
  const leftPlayerName = data.players[0].player.name;
  const rightPlayerName = data.players[1].player.name;
  let winner = null;
  ...
  Teact.useEffect(() => {
    ...
    function draw() {
      ...
      drawRect(canvas.width - PADDLE_HEIGHT, leftPaddleY, PADDLE_HEIGHT, PADDLE_WIDTH, 'white');
      if (winner === null) {
        drawBall(ballX, ballY, BALL_RADIUS, '#FFD700');
      }
      drawText(`${rightScore}`, canvas.width / 4 * 3, 50, '48px sans-serif', rightScore === 10 ? 'yellow' : 'white',);
      drawText(`${leftScore}`, canvas.width / 4, 50, '48px sans-serif', leftScore
      === 10 ? 'yellow' : 'white',);
    }
    ...
    Teact.createElement(
      ...
      Teact.createElement(
        ...
        Teact.createElement(
          ...
          Teact.createElement('div', { id: 'leftPlayer', className: 'text-center fs-2 text-white me-3', style: { writingMode: 'vertical-rl' } }, leftPlayerName),
          Teact.createElement(
            'div',
          ...
          Teact.createElement('div', { id: 'rightPlayer', className: 'text-center fs-2 text-white ms-3', style: { writingMode: 'vertical-rl' } }, rightPlayerName)
        )
      ),
      Teact.createElement('div', { className: 'd-grid gap-2 col-3 mx-auto', id: 'utilButton' }, endMatch ? DefaultButton({ text: 'トーナメント画面へ', onClick: () => fetchTournament(endMatch) }) : null)
    )
  );
};
